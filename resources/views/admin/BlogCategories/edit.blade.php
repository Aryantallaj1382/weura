...
@section('content')
    <div class="max-w-lg mx-auto p-6 bg-slate-800 rounded-lg">
        ...
        <form action="{{ route('admin.blog_categories.update', $BlogCategory->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="text-white">نام دسته‌بندی</label>
                <input type="text" name="name" value="{{ old('name', $BlogCategory->name) }}" class="w-full p-2 rounded bg-slate-900 text-white border border-slate-600">
