...
@section('content')
    <div class="p-6 space-y-6">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('admin.blog_categories.create') }}" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg">➕ افزودن</a>
        </div>
            ...
            @foreach($categories as $index => $category)
                <tr class="hover:bg-slate-700 transition">
                    ...
                    <td class="px-4 py-2 flex justify-center gap-2">
                        <a href="{{ route('admin.blog_categories.edit', $category->id) }}" class="px-3 py-1 bg-yellow-500 rounded hover:bg-yellow-600">ویرایش</a>
                        <form action="{{ route('admin.blog_categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('حذف شود؟')">
                            @csrf
                            @method('DELETE')
